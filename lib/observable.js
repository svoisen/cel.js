...
/**
 * Creates an ephemeral property. Ephemeral properties act like a
 * property, with the exception that their current value may only
 * be read once. Subsequent reads will return undefined until the
 * property has a new value.
 * @param {*} initialiValue The initial value for the ephemeral property.
 * @returns {function} A new ephemeral property.
 */
...
const filter = curryN(2, function (observable, fn) {
    return _createObservable(VALUE_POLICY.ALWAYS_EMPTY, observable._queuesValues, [observable], (val) => {
...
const scan = curryN(3, function (stream, seed, fn) {
    return combine(stream, function (val) {
        return (seed = fn(seed, val));
    });
});
...
function _attachMethods(observable) {
    observable.filter = filter(observable);
    ...
    observable.scan = scan(observable);
